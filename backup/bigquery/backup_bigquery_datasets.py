...
def main(args):
    ...
    for table in tables:
        ...
        for partition in filtered_partitions:
            ...
            partition_path = "backup/bigquery/{}/{}/{}".format(
                args.dataset, table, partition_date
            )
            partition_name = "{}:{}.{}${}".format(
                args.project, args.dataset, table, partition
            )
            ...
            if not any("schema.json" in blob for blob in blobs):
                ...
                logging.info(
                    "Starting backup for schema partition {}".format(partition)
                )
        ...
        expired = [
            ...
            if not any(
                get_date(partition) in backup for partition in filtered_partitions
            )
        ]
...
def get_schema(partition_name):
    ...
    schema = exec_shell_command(["bq", "show", "--schema", partition_name])
...
def backup_schema(client, bucket_name, file_name, schema):
    ...
    blob.upload_from_string(schema, content_type="application/json")
...
def get_partitions(client, project, dataset, table):
    ...
    query = """
        SELECT
          partition_id
        FROM [{}.{}.{}$__PARTITIONS_SUMMARY__]""".format(  # nosec
...
def get_partition_size(client, partition_name):
    ...
    job_config = bigquery.QueryJobConfig(
        use_legacy_sql=True, use_query_cache=False, dry_run=True
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default="EU",
        help="location of a bigquery dataset",
    )
