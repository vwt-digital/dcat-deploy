...
for pr in $(python3 "${basedir}"/listrepos.py "${DATA_CATALOG}"); do
  ...
  curl -i -X PUT -H "Authorization:token ${GITHUB_ACCESS_TOKEN}" "https://api.github.com/repos/${ORGANISATION}/${REPO_NAME}/contents/.github/workflows/validation.yaml" -d '{"message":"Initial validation","content":"'"$(less -FX "${basedir}"/validation.b64)"'"}'
  curl -i -X PUT -H "Authorization:token ${GITHUB_ACCESS_TOKEN}" "https://api.github.com/repos/${ORGANISATION}/${REPO_NAME}/contents/CONTRIBUTING.md" -d '{"message":"Initial contributing","content":"'"$(less -FX "${basedir}"/contributing.b64)"'"}'
